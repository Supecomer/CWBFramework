...
MGSYNTH_DUMMY_CLASS(UIDevice_MGAdd)


@implementation UIDevice (MGAdd)
...
typedef struct {
    ...
    uint64_t awdl_in;
    uint64_t awdl_out;
} MG_net_interface_counter;


static uint64_t MG_net_counter_add(uint64_t counter, uint64_t bytes) {
...
static uint64_t MG_net_counter_get_by_type(MG_net_interface_counter *counter, MGNetworkTrafficType type) {
    uint64_t bytes = 0;
    if (type & MGNetworkTrafficTypeWWANSent) bytes += counter->pdp_ip_out;
    if (type & MGNetworkTrafficTypeWWANReceived) bytes += counter->pdp_ip_in;
    if (type & MGNetworkTrafficTypeWIFISent) bytes += counter->en_out;
    if (type & MGNetworkTrafficTypeWIFIReceived) bytes += counter->en_in;
    if (type & MGNetworkTrafficTypeAWDLSent) bytes += counter->awdl_out;
    if (type & MGNetworkTrafficTypeAWDLReceived) bytes += counter->awdl_in;
    return bytes;
}

static MG_net_interface_counter MG_get_net_interface_counter() {
    ...
    MG_net_interface_counter counter = {0};
        ...
        while (cursor) {
            if (cursor->ifa_addr->sa_family == AF_LINK) {
                ...
                if (name) {
                    ...
                    counter_in = MG_net_counter_add(counter_in, data->ifi_ibytes);
                    sharedInCounters[name] = @(counter_in);
                    
                    uint64_t counter_out = ((NSNumber *)sharedOutCounters[name]).unsignedLongLongValue;
                    counter_out = MG_net_counter_add(counter_out, data->ifi_obytes);
...
- (uint64_t)getNetworkTrafficBytes:(MGNetworkTrafficType)types {
    MG_net_interface_counter counter = MG_get_net_interface_counter();
    return MG_net_counter_get_by_type(&counter, types);
}
