...
#import "MGYYFrameImage.h"
#import "MGYYSpriteSheetImage.h"
...
/**
 A MGYYImage object is a high-level way to display animated image data.
 ...
 animation, try replace UIImageView with `MGYYAnimatedImageView`.
 
 Sample Code:
 
     // animation@3x.webp
     MGYYImage *image = [MGYYImage imageNamed:@"animation.webp"];
     MGYYAnimatedImageView *imageView = [MGYYAnimatedImageView alloc] initWithImage:image];
     [view addSubView:imageView];
    
 */
@interface MGYYImage : UIImage <MGYYAnimatedImage>

+ (nullable MGYYImage *)imageNamed:(NSString *)name; // no cache!
+ (nullable MGYYImage *)imageWithContentsOfFile:(NSString *)path;
+ (nullable MGYYImage *)imageWithData:(NSData *)data;
+ (nullable MGYYImage *)imageWithData:(NSData *)data scale:(CGFloat)scale;

/**
 If the image is created from data or file, then the value indicates the data type.
 */
@property (nonatomic, readonly) MGYYImageType animatedImageType;
