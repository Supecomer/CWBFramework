...
+ (MGYYImage *)imageWithContentsOfFile:(NSString *)path {
    return [[self alloc] initWithContentsOfFile:path];
}

+ (MGYYImage *)imageWithData:(NSData *)data {
    return [[self alloc] initWithData:data];
}

+ (MGYYImage *)imageWithData:(NSData *)data scale:(CGFloat)scale {
    return [[self alloc] initWithData:data scale:scale];
}
...
- (instancetype)initWithData:(NSData *)data scale:(CGFloat)scale {
    ...
    @autoreleasepool {
        ...
        MGYYImageFrame *frame = [decoder frameAtIndex:0 decodeForDisplay:YES];
        ...
        self.MGYY_isDecodedForDisplay = YES;
    }
    return self;
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    NSNumber *scale = [aDecoder decodeObjectForKey:@"MGYYImageScale"];
    NSData *data = [aDecoder decodeObjectForKey:@"MGYYImageData"];
    if (data.length) {
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    if (_decoder.data.length) {
        [aCoder encodeObject:@(self.scale) forKey:@"MGYYImageScale"];
        [aCoder encodeObject:_decoder.data forKey:@"MGYYImageData"];
    } else {
...
#pragma mark - protocol MGYYAnimatedImage
