...
/**
 ...
 If the `image` or `highlightedImage` property adopt to the `MGYYAnimatedImage` protocol,
...
/**
 The MGYYAnimatedImage protocol declares the required methods for animated image
 display with MGYYAnimatedImageView.
 
 Subclass a UIImage and implement this protocol, so that instances of that class 
 can be set to MGYYAnimatedImageView.image or MGYYAnimatedImageView.highlightedImage
 to display animation.
 
 See `MGYYImage` and `MGYYFrameImage` for example.
 */
@protocol MGYYAnimatedImage <NSObject>
@required
