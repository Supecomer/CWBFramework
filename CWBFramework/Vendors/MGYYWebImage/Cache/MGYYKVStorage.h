...
/**
 MGYYKVStorageItem is used by `MGYYKVStorage` to store key-value pair and meta data.
 Typically, you should not use this class directly.
 */
@interface MGYYKVStorageItem : NSObject
...
/**
 Storage type, indicated where the `MGYYKVStorageItem.value` stored.
 ...
 * If you want to store large number of small datas (such as contacts cache), 
   use MGYYKVStorageTypeSQLite to get better performance.
 * If you want to store large files (such as image cache),
   use MGYYKVStorageTypeFile to get better performance.
 * You can use MGYYKVStorageTypeMixed and choice your storage type for each item.
 
 See <http://www.sqlite.org/intern-v-extern-blob.html> for more information.
 */
typedef NS_ENUM(NSUInteger, MGYYKVStorageType) {
    
    /// The `value` is stored as a file in file system.
    MGYYKVStorageTypeFile = 0,
    
    /// The `value` is stored in sqlite with blob type.
    MGYYKVStorageTypeSQLite = 1,
    
    /// The `value` is stored in file system or sqlite based on your choice.
    MGYYKVStorageTypeMixed = 2,
};



/**
 MGYYKVStorage is a key-value storage based on sqlite and file system.
 Typically, you should not use this class directly.
 
 @discussion The designated initializer for MGYYKVStorage is `initWithPath:type:`. 
 ...
 */
@interface MGYYKVStorage : NSObject
...
@property (nonatomic, readonly) MGYYKVStorageType type;  ///< The type of this storage.
 ...
 */
- (nullable instancetype)initWithPath:(NSString *)path type:(MGYYKVStorageType)type NS_DESIGNATED_INITIALIZER;
...
/**
 ...
 If the `type` is MGYYKVStorageTypeFile, then the item.filename should not be empty.
 If the `type` is MGYYKVStorageTypeSQLite, then the item.filename will be ignored.
 It the `type` is MGYYKVStorageTypeMixed, then the item.value will be saved to file 
 system if the item.filename is not empty, otherwise it will be saved to sqlite.
 
 @param item  An item.
 @return Whether succeed.
 */
- (BOOL)saveItem:(MGYYKVStorageItem *)item;

/**
 Save an item or update the item with 'key' if it already exists.
 
 @discussion This method will save the key-value pair to sqlite. If the `type` is
 MGYYKVStorageTypeFile, then this method will failed.
...
/**
 ...
 @discussion
 If the `type` is MGYYKVStorageTypeFile, then the `filename` should not be empty.
 If the `type` is MGYYKVStorageTypeSQLite, then the `filename` will be ignored.
 It the `type` is MGYYKVStorageTypeMixed, then the `value` will be saved to file
 ...
 */
- (nullable MGYYKVStorageItem *)getItemForKey:(NSString *)key;
 ...
 */
- (nullable MGYYKVStorageItem *)getItemInfoForKey:(NSString *)key;
...
/**
 ...
 @param keys  An array of specified keys.
 @return An array of `MGYYKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<MGYYKVStorageItem *> *)getItemForKeys:(NSArray<NSString *> *)keys;
...
/**
 ...
 @return An array of `MGYYKVStorageItem`, or nil if not exists / error occurs.
 */
- (nullable NSArray<MGYYKVStorageItem *> *)getItemInfoForKeys:(NSArray<NSString *> *)keys;
