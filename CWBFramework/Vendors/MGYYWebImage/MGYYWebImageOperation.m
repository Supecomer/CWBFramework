...
static UIApplication *_MGYYSharedApplication() {
...
static BOOL MGYYCGImageLastPixelFilled(CGImageRef image) {
    if (!image) return NO;
    size_t width = CGImageGetWidth(image);
    size_t height = CGImageGetHeight(image);
    if (width == 0 || height == 0) return NO;
    CGContextRef ctx = CGBitmapContextCreate(NULL, 1, 1, 8, 0, MGYYCGColorSpaceGetDeviceRGB(), kCGImageAlphaPremultipliedFirst | kCGBitmapByteOrderDefault);
    if (!ctx) return NO;
...
@interface _MGYYWebImageWeakProxy : NSProxy
@property (nonatomic, weak, readonly) id target;
- (instancetype)initWithTarget:(id)target;
+ (instancetype)proxyWithTarget:(id)target;
@end

@implementation _MGYYWebImageWeakProxy
- (instancetype)initWithTarget:(id)target {
    _target = target;
    return self;
}
+ (instancetype)proxyWithTarget:(id)target {
    return [[_MGYYWebImageWeakProxy alloc] initWithTarget:target];
}
...
@interface MGYYWebImageOperation() <NSURLConnectionDelegate>
...
@property (nonatomic, strong) MGYYImageDecoder *progressiveDecoder;
...
@property (nonatomic, copy) MGYYWebImageProgressBlock progress;
@property (nonatomic, copy) MGYYWebImageTransformBlock transform;
@property (nonatomic, copy) MGYYWebImageCompletionBlock completion;
@end


@implementation MGYYWebImageOperation
...
- (instancetype)init {
    @throw [NSException exceptionWithName:@"MGYYWebImageOperation init error" reason:@"MGYYWebImageOperation must be initialized with a request. Use the designated initializer to init." userInfo:nil];
...
- (void)dealloc {
    ...
    if (_taskID != UIBackgroundTaskInvalid) {
        [_MGYYSharedApplication() endBackgroundTask:_taskID];
        _taskID = UIBackgroundTaskInvalid;
    }
    if ([self isExecuting]) {
        self.cancelled = YES;
        ...
        if (_connection) {
            [_connection cancel];
            if (![_request.URL isFileURL] && (_options & MGYYWebImageOptionShowNetworkActivity)) {
                [MGYYWebImageManager decrementNetworkActivityCount];
            }
        }
        if (_completion) {
            @autoreleasepool {
                _completion(nil, _request.URL, MGYYWebImageFromNone, MGYYWebImageStageCancelled, nil);
            }
        }
    }
    [_lock unlock];
...
- (void)_startOperation {
    ...
    @autoreleasepool {
        ...
        if (_cache &&
            !(_options & MGYYWebImageOptionUseNSURLCache) &&
            !(_options & MGYYWebImageOptionRefreshImageCache)) {
            UIImage *image = [_cache getImageForKey:_cacheKey withType:MGYYImageCacheTypeMemory];
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    if (_completion) _completion(image, _request.URL, MGYYWebImageFromMemoryCache, MGYYWebImageStageFinished, nil);
                }
                [self _finish];
                [_lock unlock];
                return;
            }
            if (!(_options & MGYYWebImageOptionIgnoreDiskCache)) {
                __weak typeof(self) _self = self;
                dispatch_async([self.class _imageQueue], ^{
                    __strong typeof(_self) self = _self;
                    if (!self || [self isCancelled]) return;
                    UIImage *image = [self.cache getImageForKey:self.cacheKey withType:MGYYImageCacheTypeDisk];
                    if (image) {
                        [self.cache setImage:image imageData:nil forKey:self.cacheKey withType:MGYYImageCacheTypeMemory];
...
- (void)_startRequest:(id)object {
    if ([self isCancelled]) return;
    @autoreleasepool {
        if ((_options & MGYYWebImageOptionIgnoreFailedURL) && URLBlackListContains(_request.URL)) {
            ...
            [_lock lock];
            if (![self isCancelled]) {
                if (_completion) _completion(nil, _request.URL, MGYYWebImageFromNone, MGYYWebImageStageFinished, error);
            }
            [self _finish];
            [_lock unlock];
            return;
        ...
        [_lock lock];
        if (![self isCancelled]) {
            _connection = [[NSURLConnection alloc] initWithRequest:_request delegate:[_MGYYWebImageWeakProxy proxyWithTarget:self]];
            if (![_request.URL isFileURL] && (_options & MGYYWebImageOptionShowNetworkActivity)) {
                [MGYYWebImageManager incrementNetworkActivityCount];
            }
        }
        [_lock unlock];
    }
...
- (void)_cancelOperation {
    @autoreleasepool {
        ...
        [_connection cancel];
        _connection = nil;
        if (_completion) _completion(nil, _request.URL, MGYYWebImageFromNone, MGYYWebImageStageCancelled, nil);
        [self _endBackgroundTask];
    }
}
...
- (void)_didReceiveImageFromDiskCache:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (image) {
                if (_completion) _completion(image, _request.URL, MGYYWebImageFromDiskCache, MGYYWebImageStageFinished, nil);
                [self _finish];
            } else {
...
- (void)_didReceiveImageFromWeb:(UIImage *)image {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_cache) {
                if (image || (_options & MGYYWebImageOptionRefreshImageCache)) {
                    NSData *data = _data;
                    dispatch_async([MGYYWebImageOperation _imageQueue], ^{
                        MGYYImageCacheType cacheType = (_options & MGYYWebImageOptionIgnoreDiskCache) ? MGYYImageCacheTypeMemory : MGYYImageCacheTypeAll;
            ...
            if (_completion) _completion(image, _request.URL, MGYYWebImageFromRemote, MGYYWebImageStageFinished, error);
            [self _finish];
        }
        [_lock unlock];
    }
...
- (void)connection:(NSURLConnection *)connection willSendRequestForAuthenticationChallenge:(NSURLAuthenticationChallenge *)challenge {
    @autoreleasepool {
        if ([challenge.protectionSpace.authenticationMethod isEqualToString:NSURLAuthenticationMethodServerTrust]) {
            if (!(_options & MGYYWebImageOptionAllowInvalidSSLCertificates) &&
...
- (NSCachedURLResponse *)connection:(NSURLConnection *)connection willCacheResponse:(NSCachedURLResponse *)cachedResponse {
    ...
    if (_options & MGYYWebImageOptionUseNSURLCache) {
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        /*--------------------------- progressive ----------------------------*/
        BOOL progressive = (_options & MGYYWebImageOptionProgressive) > 0;
        BOOL progressiveBlur = (_options & MGYYWebImageOptionProgressiveBlur) > 0;
        ...
        if (!_progressiveDecoder) {
            _progressiveDecoder = [[MGYYImageDecoder alloc] initWithScale:[UIScreen mainScreen].scale];
        }
        [_progressiveDecoder updateData:_data final:NO];
        if ([self isCancelled]) return;
        
        if (_progressiveDecoder.type == MGYYImageTypeUnknown ||
            _progressiveDecoder.type == MGYYImageTypeWebP ||
            _progressiveDecoder.type == MGYYImageTypeOther) {
            _progressiveDecoder = nil;
            _progressiveIgnored = YES;
            return;
        }
        if (progressiveBlur) { // only support progressive JPEG and interlaced PNG
            if (_progressiveDecoder.type != MGYYImageTypeJPEG &&
                _progressiveDecoder.type != MGYYImageTypePNG) {
                _progressiveDecoder = nil;
                _progressiveIgnored = YES;
                return;
            }
        ...
        if (!progressiveBlur) {
            MGYYImageFrame *frame = [_progressiveDecoder frameAtIndex:0 decodeForDisplay:YES];
            if (frame.image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(frame.image, _request.URL, MGYYWebImageFromRemote, MGYYWebImageStageProgress, nil);
            ...
            } else if (_progressiveDecoder.type == MGYYImageTypePNG) {
                if (!_progressiveDetected) {
                    NSDictionary *dic = [_progressiveDecoder framePropertiesAtIndex:0];
            ...
            if (!MGYYCGImageLastPixelFilled(image.CGImage)) return;
            ...
            image = [image MGYY_imageByBlurRadius:radius tintColor:nil tintMode:0 saturation:1 maskImage:nil];
            
            if (image) {
                [_lock lock];
                if (![self isCancelled]) {
                    _completion(image, _request.URL, MGYYWebImageFromRemote, MGYYWebImageStageProgress, nil);
                    _lastProgressiveDecodeTimestamp = now;
                }
                [_lock unlock];
            }
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                BOOL shouldDecode = (self.options & MGYYWebImageOptionIgnoreImageDecoding) == 0;
                BOOL allowAnimation = (self.options & MGYYWebImageOptionIgnoreAnimatedImage) == 0;
                UIImage *image;
                BOOL hasAnimation = NO;
                if (allowAnimation) {
                    image = [[MGYYImage alloc] initWithData:self.data scale:[UIScreen mainScreen].scale];
                    if (shouldDecode) image = [image MGYY_imageByDecoded];
                    if ([((MGYYImage *)image) animatedImageFrameCount] > 1) {
                        hasAnimation = YES;
                    }
                } else {
                    MGYYImageDecoder *decoder = [MGYYImageDecoder decoderWithData:self.data scale:[UIScreen mainScreen].scale];
                 ...
                 */
                MGYYImageType imageType = MGYYImageDetectType((__bridge CFDataRef)self.data);
                switch (imageType) {
                    case MGYYImageTypeJPEG:
                    case MGYYImageTypeGIF:
                    case MGYYImageTypePNG:
                    case MGYYImageTypeWebP: { // save to disk cache
                        if (!hasAnimation) {
                            if (imageType == MGYYImageTypeGIF ||
                                imageType == MGYYImageTypeWebP) {
            ...
            if (![self.request.URL isFileURL] && (self.options & MGYYWebImageOptionShowNetworkActivity)) {
                [MGYYWebImageManager decrementNetworkActivityCount];
            }
        }
        [_lock unlock];
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            if (_completion) {
                _completion(nil, _request.URL, MGYYWebImageFromNone, MGYYWebImageStageFinished, error);
            }
            _connection = nil;
            _data = nil;
            if (![_request.URL isFileURL] && (_options & MGYYWebImageOptionShowNetworkActivity)) {
    ...
    @autoreleasepool {
        ...
        if ([self isCancelled]) {
            ...
            if (!_request) {
                ...
                if ((_options & MGYYWebImageOptionAllowBackgroundTask) && _MGYYSharedApplication()) {
                    __weak __typeof__ (self) _self = self;
                    if (_taskID == UIBackgroundTaskInvalid) {
                        _taskID = [_MGYYSharedApplication() beginBackgroundTaskWithExpirationHandler:^{
