//
//  MGYYWebImageManager.h
//  MGYYWebImage <MGYYWebImage>
//
//  Created by ciome on 15/2/19.
...
typedef NS_ENUM(NSUInteger, MGYYWebImageFromType) {
    ...
    MGYYWebImageFromMemoryCacheFast,
    
    /// Fetched from memory cache.
    MGYYWebImageFromMemoryCache,
    
    /// Fetched from disk cache.
    MGYYWebImageFromDiskCache,
    
    /// Fetched from remote (web or file path).
    MGYYWebImageFromRemote,
};

/// Indicated image fetch complete stage.
typedef NS_ENUM(NSInteger, MGYYWebImageStage) {
    
    /// Incomplete, progressive image.
    MGYYWebImageStageProgress  = -1,
    
    /// Cancelled.
    MGYYWebImageStageCancelled = 0,
    
    /// Finished (succeed or failed).
    MGYYWebImageStageFinished  = 1,
};
 ...
 */
typedef void(^MGYYWebImageProgressBlock)(NSInteger receivedSize, NSInteger expectedSize);

/**
 The block invoked before remote image fetch finished to do additional image process.
 
 @discussion This block will be invoked before `MGYYWebImageCompletionBlock` to give
 ...
 @example You can clip the image, blur it and add rounded corners with these code:
    ^(UIImage *image, NSURL *url) {
        ...
        image = [image MGYY_imageByResizeToSize:CGSizeMake(100, 100) contentMode:UIViewContentModeScaleAspectFill];
        image = [image MGYY_imageByBlurRadius:20 tintColor:nil tintMode:kCGBlendModeNormal saturation:1.2 maskImage:nil];
        image = [image MGYY_imageByRoundCornerRadius:5];
        return image;
    }
 ...
 */
typedef UIImage * _Nullable (^MGYYWebImageTransformBlock)(UIImage *image, NSURL *url);
 ...
 */
typedef void (^MGYYWebImageCompletionBlock)(UIImage * _Nullable image,
                                          NSURL *url,
                                          MGYYWebImageFromType from,
                                          MGYYWebImageStage stage,
 ...
 */
@interface MGYYWebImageManager : NSObject

/**
 Returns global MGYYWebImageManager instance.
 
 @return MGYYWebImageManager shared instance.
 */
 ...
 */
- (instancetype)initWithCache:(nullable MGYYImageCache *)cache
 ...
 */
- (nullable MGYYWebImageOperation *)requestImageWithURL:(NSURL *)url
 ...
 */
@property (nullable, nonatomic, strong) MGYYImageCache *cache;
 ...
 */
@property (nullable, nonatomic, copy) MGYYWebImageTransformBlock sharedTransformBlock;
...
/**
 ...
 @param url A specified URL
 @return Cache key used in MGYYImageCache.
 */
