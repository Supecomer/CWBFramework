...
@class MGYYMemoryCache, MGYYDiskCache;

NS_ASSUME_NONNULL_BEGIN

/// Image cache type
typedef NS_OPTIONS(NSUInteger, MGYYImageCacheType) {
    /// No value.
    MGYYImageCacheTypeNone   = 0,
    
    /// Get/store image with memory cache.
    MGYYImageCacheTypeMemory = 1 << 0,
    
    /// Get/store image with disk cache.
    MGYYImageCacheTypeDisk   = 1 << 1,
    
    /// Get/store image with both memory cache and disk cache.
    MGYYImageCacheTypeAll    = MGYYImageCacheTypeMemory | MGYYImageCacheTypeDisk,
};


/**
 MGYYImageCache is a cache that stores UIImage and image data based on memory cache and disk cache.
 ...
 */
@interface MGYYImageCache : NSObject
...
/** The underlying memory cache. see `MGYYMemoryCache` for more information.*/
@property (strong, readonly) MGYYMemoryCache *memoryCache;

/** The underlying disk cache. see `MGYYDiskCache` for more information.*/
@property (strong, readonly) MGYYDiskCache *diskCache;

/**
 Whether decode animated image when fetch image from disk cache. Default is YES.
 
 @discussion When fetch image from disk cache, it will use 'MGYYImage' to decode 
...
/**
 ...
 @return  The singleton MGYYImageCache instance.
 */
...
/**
 ...
 @discussion If the `type` contain `MGYYImageCacheTypeMemory`, then the `image` will 
 be stored in the memory cache; `imageData` will be used instead if `image` is nil.
 If the `type` contain `MGYYImageCacheTypeDisk`, then the `imageData` will
...
- (void)setImage:(nullable UIImage *)image
       imageData:(nullable NSData *)imageData
          forKey:(NSString *)key
        withType:(MGYYImageCacheType)type;
...
/**
 Returns a Boolean value that indicates whether a given key is in cache.
 If the image is not in memory and the `type` contains `MGYYImageCacheTypeDisk`,
...
/**
 ...
 @return The image associated with key, or nil if no image is associated with key.
 */
- (nullable UIImage *)getImageForKey:(NSString *)key withType:(MGYYImageCacheType)type;
...
- (void)getImageForKey:(NSString *)key
              withType:(MGYYImageCacheType)type
             withBlock:(void(^)(UIImage * _Nullable image, MGYYImageCacheType type))block;
