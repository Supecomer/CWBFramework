...
#define MGMBMainThreadAssert() NSAssert([NSThread isMainThread], @"MGMBProgressHUD needs to be accessed on the main thread.");

CGFloat const MGMBProgressMaxOffset = 1000000.f;

static const CGFloat MGMBDefaultPadding = 4.f;
static const CGFloat MGMBDefaultLabelFontSize = 16.f;
static const CGFloat MGMBDefaultDetailsLabelFontSize = 12.f;
...
@interface MGMBProgressHUDRoundedButton : UIButton
@end
...
+ (MGMBProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            MGMBProgressHUD *hud = (MGMBProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = MGMBProgressHUDAnimationFade;
    _mode = MGMBProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(MGMBProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == MGMBProgressHUDAnimationZoom) {
        type = animatingIn ? MGMBProgressHUDAnimationZoomIn : MGMBProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == MGMBProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == MGMBProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == MGMBProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == MGMBProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
...
- (void)done {
    ...
    MGMBProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<MGMBProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    MGMBBackgroundView *backgroundView = [[MGMBBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = MGMBProgressHUDBackgroundStyleSolidColor;
    ...
    MGMBBackgroundView *bezelView = [MGMBBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:MGMBDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:MGMBDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [MGMBProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:MGMBDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    MGMBProgressHUDMode mode = self.mode;
    if (mode == MGMBProgressHUDModeIndeterminate) {
        ...
        [indicator removeFromSuperview];
        indicator = [[MGMBBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
        ...
        if (!isRoundIndicator) {
            ...
            [indicator removeFromSuperview];
            indicator = [[MGMBRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
    ...
    else if (mode == MGMBProgressHUDModeCustomView && self.customView != indicator) {
...
- (void)updateViewsForColor:(UIColor *)color {
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[MGMBProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[MGMBProgressHUD class]]];
#endif
        
        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[MGMBRoundProgressView class]]) {
        MGMBRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MGMBRoundProgressView appearanceWhenContainedIn:[MGMBProgressHUD class], nil];
#else
        appearance = [MGMBRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[MGMBProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((MGMBRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((MGMBRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[MGMBBarProgressView class]]) {
        MGMBBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [MGMBBarProgressView appearanceWhenContainedIn:[MGMBProgressHUD class], nil];
#else
        appearance = [MGMBBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[MGMBProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((MGMBBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((MGMBBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    MGMBBackgroundView *bezelView = self.bezelView;
...
- (void)layoutSubviews {
    ...
    // would trigger a zoMGMBie object access.
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? MGMBDefaultPadding : 0.f;
...
- (void)setMode:(MGMBProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == MGMBProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation MGMBBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = MGMBProgressHUDBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
            ...
            _style = MGMBProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(MGMBProgressHUDBackgroundStyle)style {
    if (style == MGMBProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = MGMBProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    MGMBProgressHUDBackgroundStyle style = self.style;
    if (style == MGMBProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == MGMBProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation MGMBProgressHUDRoundedButton
