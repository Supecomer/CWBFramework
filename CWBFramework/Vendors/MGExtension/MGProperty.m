...
{
    MGProperty *propertyObj = objc_getAssociatedObject(self, property);
    if (propertyObj == nil) {
...
- (void)setProperty:(objc_property_t)property
{
    ...
    MGExtensionAssertParamNotNil(property);
    ...
    _type = [MGPropertyType cachedTypeWithCode:code];
}
...
- (NSArray *)propertyKeysWithStringKey:(NSString *)stringKey
{
    ...
    for (NSString *oldKey in oldKeys) {
        ...
        if (start != NSNotFound) { // 有索引的key
            ...
            if (prefixKey.length) {
                MGPropertyKey *propertyKey = [[MGPropertyKey alloc] init];
            ...
            for (NSInteger i = 0; i<cmps.count - 1; i++) {
                MGPropertyKey *subPropertyKey = [[MGPropertyKey alloc] init];
                subPropertyKey.type = MGPropertyKeyTypeArray;
